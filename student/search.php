		...
		<div id="container">
			<div id="header">
				...
				<div id="headerNavigation">
					<a href="index.php?page=notification" onclick="clearNotif()"><div id="notificationDiv" class="navRow"><span><h6 id="notifStat"></h6>Notifications</span></div></a>
					...
					<a href="index.php?page=messages" onclick="clearMessage()"><div id="notificationDiv" class="navRow"><span><h6 id="messageStat"></h6>Messages</span></div></a>
			...
			<div id="body">
				<div id="searchBar">
					...
					<form action="search.php" method="GET">
						...
						<input type="text" name="keyword" placeholder="Search A Job" id="searchField"/>
						<input type="submit" value="Search Now" id="button"/>
					
				</div>
				<div id="filters">
					<div id="sorting">
						<span><img src="../images/sort.png"/>&nbsp;&nbsp;Sort Results By Job Title</span><br/><br/>
						<a><button class="left" type="button" onclick="location.href='search.php?keyword=<?php echo $keyword; ?>&skill=<?php echo $skill; ?>&location=<?php echo $location; ?>' ">A - Z</button></a><a><button class="right" type="button" onclick="location.href='search.php?keyword=<?php echo $keyword; ?>&skill=<?php echo $skill; ?>&location=<?php echo $location; ?>&sort=DESC' ">Z - A</button></a><br/><br/>
						<span><img src="../images/filter.png"/>&nbsp;&nbsp;Filter Results</span><br/><br/>
						<input type="text" name="skill" placeholder="Special Skills" id="skillField"/><br/><br/>
						<input type="text" name="location" placeholder="Location" id="skillField"/><br/>
					</form>
					</div>
					<div id="guide">
				...
				<div id="results">
					<div id="banner"><img src="../images/resultslogan.png"/><span id="job"><?php echo $keyword." ".(($skill != '' ) ? ' - '.$skill : '') ?></span><span id="percentage"><?php /*echo ($search->student_results($profession->get_profession_id($keyword),$skill)['result']." / ".$search->student_results($profession->get_profession_id($keyword),$skill)['total'])*/ ?> <!--Matches!--></span></div>
					<div id="innerResults">
						<?php
							foreach($search->student_search($keyword,$skill,$_SESSION['status'],$sort,$location) as $jobs) {
								echo "<div class='row'>
											<div id='jobName'><a href='detail.php?page=jobdetail&job_id=".$jobs['job_id']."&company_id=".$job->get_employer_id($jobs['job_id'])."'><span id='jobText'>".$profession->get_profession_name($job->get_profession_id($jobs['job_id']))."</span></a><!--<a href='' onclick=saveJob('".$jobs['job_id']."','".$_SESSION['user_id']."') id='saveJob'><div><span>Save Job</span><img src='../images/savejob.png'/></div></a>!--></div>
											<div id='companyName'><a href='detail.php?page=companydetail&company_id=".$job->get_employer_id($jobs['job_id'])." '><span id='companyText'>".$company->get_name($job->get_employer_id($jobs['job_id']))."</span></a><!--<a href='' onclick=applyJob('".$jobs['job_id']."','".$_SESSION['user_id']."') id='applyJob'><div><span>Apply</span><img src='../images/applyjob.png'/></div></a>!--></div>
											<div id='description'>".$job->get_description($jobs['job_id'])."</div>
										</div>";
