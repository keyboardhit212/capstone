		...
		<?php
			foreach($message->get_message($_SESSION['to'],$_SESSION['user_id']) as $messages) {
				echo "<div id='messageRow'>
							&nbsp;&nbsp;<img src='".$user->get_pic($messages['message_from'])."'/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span id='messageName'>".$user->get_full_name($messages['message_from'])."<br/><span id='date'>".$messages['message_date_received']." | ".$messages['message_time_received']."</span></span>
							<span id='textMessage'>".$messages['message']."</span>
						</div>";
				//$messages['message_from']."<br/>".$messages['message']."<br/><br/>";
			}
		?>
