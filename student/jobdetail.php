	...
	<body>
		...
		<div id="companyDetails">
			<div id="pic">
				...
				<a href="<?php echo $company->get_url($company_id); ?>"><?php echo $company->get_url($company_id); ?></a>
			</div>
		...
		<div id="jobDetail">
			...
			<a href=""  onclick="return <?php echo (($applicant->is_applicant($job_id,$_SESSION['user_id']) || $applicant->is_company_applicant($company_id, $_SESSION['user_id'])) ? 'false'  : 'applyJob('.$job_id.','.$_SESSION['user_id'].','.$company_id.')') ?> "><div id="closeJob" <?php echo (($applicant->is_applicant($job_id,$_SESSION['user_id']) || $applicant->is_company_applicant($company_id, $_SESSION['user_id'])) ? "style=background:red" : "") ?>><span><?php echo (($applicant->is_applicant($job_id,$_SESSION['user_id'])) ? 'Applied' : 'Apply') ?></span></div></a>
			<a href="" onclick="return <?php echo (($jobcart->is_saved($job_id,$_SESSION['user_id'])) ? 'false' : 'saveJob('.$job_id.','.$_SESSION['user_id'].')') ?>"><div id="closeJob" <?php echo (($jobcart->is_saved($job_id,$_SESSION['user_id'])) ? 'style=background:red' : '') ?>><span><?php echo (($jobcart->is_saved($job_id,$_SESSION['user_id'])) ? 'Saved' : 'Save Job') ?></span></div></a>
		...
		<div id="skills"><span id="label">Special Skills: <span id="content"><?php echo $job->get_skill($job_id); ?></span></span></div>
		<div id="skills"><span id="label">Salary: <span id="content"><?php echo $job->get_salary($job_id); ?></span></span></div>
		<div id="skills"><span id="label">Location: <span id="content"><?php echo $job->get_location($job_id); ?></span></span></div>
		<div id="skills"><span id="label">Posted: <span id="content"><?php echo DateTime::createFromFormat('Y-m-d',$job->get_date_added($job_id))->format('m/d/Y'); ?></span></span></div>
	</body>
</html>